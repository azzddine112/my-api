...
app.get('/api/extract', async (req, res) => {
    ...
    if (url.includes("facebook.com") || url.includes("fb.watch") || url.includes("fb.com") || url.includes("share") || url.includes("reel")) {
        ...
        // المحاولة الأولى: استخدام محرك Cobalt السريع لـ Reels
        try {
            ...
            if (cData) {
                if (cData.status === 'stream' || cData.status === 'redirect') {
                    return res.json({
                        success: true,
                        video_url: cData.url,
                        thumbnail: cData.picker && cData.picker[0] ? cData.picker[0].thumb : ''
                    });
        ...
        // المحاولة الثانية: محرك SnapSave لروابط الفيسبوك
        try {
